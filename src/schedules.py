...
def _validate_references(payload: Dict[str, Any]) -> None:
    ...
    _ensure_collaborator(payload["instrutor_id"], "instrutor")
    _ensure_collaborator(payload["analista_id"], "analista")
    _ensure_collaborator(payload["assistente_id"], "assistente")
...
def _ensure_collaborator(collaborator_id: str, expected_type: str) -> None:
    collaborators = load_items(INSTRUCTORS_FILE)
    collaborator = find_item(collaborators, collaborator_id)
    if not collaborator:
        raise ValidationError(f"Colaborador não encontrado: {collaborator_id}")
    if collaborator.get("tipo") != expected_type:
        raise ValidationError(
            f"Colaborador inválido para {expected_type}: {collaborator_id}"
        )
